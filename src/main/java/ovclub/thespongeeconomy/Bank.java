...
import ovclub.thespongeeconomy.event.BalanceChangeEvent;
...
public class Bank {
    ...
    public Bank(TheSpongeEconomy theSpongeEconomy, EconomyImplementer eco) {
        this.eco = eco;
        balanceFile = new Json("balance.json", theSpongeEconomy.getDataFolder() + "/data/");
        playerBank = new HashMap<>();
        fakeAccountsFile = new Json("fakeAccounts.json", theSpongeEconomy.getDataFolder() + "/data/");
    ...
    public void setBalance(String uuid, int balance){
        ...
        BalanceChangeEvent event = new BalanceChangeEvent(Bukkit.getPlayer(uuid), balance);
